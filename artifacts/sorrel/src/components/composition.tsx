...
/** Framed 9:16 surface with scrub + play — the Studio live preview. */
export function CompositionPlayer({
  ...
  const [t, setT] = React.useState(0.58);
  ...
  const tc = (x: number) => `0:${Math.floor(x * DUR).toString().padStart(2, "0")}`;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, width: "100%" }}>
      ...
      >
        <Composition t={t} vars={vars} brand={brand} accent={accent} bg={bg} layout={layout} />
        <button
...
/** Loading shimmer placeholder sized like the player. */
export function CompositionLoading() {
  return (
    <div
      className="flex aspect-[9/16] w-full items-center justify-center rounded-[18px] border"
