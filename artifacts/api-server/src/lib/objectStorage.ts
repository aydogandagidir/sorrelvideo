...
export class ObjectStorageService {
  ...
  /**
   * SERVER-side upload of generated bytes (e.g. talking-host TTS audio) into
   * the private dir, mirroring the client 2-phase flow's layout
   * (`uploads/<uuid>`) and finalize semantics: the ACL policy is stamped
   * immediately, because an object with NO policy fails closed on read.
   * Returns the canonical "/objects/uploads/<uuid>" entity path.
   */
  async uploadPrivateObject(
    ...
    await this.trySetObjectEntityAclPolicy(objectPath, acl);
    return objectPath;
  }
