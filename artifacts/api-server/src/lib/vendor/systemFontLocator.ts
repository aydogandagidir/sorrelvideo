/**
 * VENDORED from the Hyperframes monorepo (Apache-2.0):
 *   https://github.com/heygen-com/hyperframes/blob/c52165d1b63cf11955ceb4e2265cbe34b0718852/packages/core/src/fonts/systemFontLocator.ts
 *   (@hyperframes/core 0.6.91 — the package SHIPS this module in dist/ but does
 *   not list it in its `exports` map, and Node forbids `imports`/`exports`
 *   targets that reach into node_modules, so a faithful copy is the only way to
 *   consume it without resolution tricks. Same treatment as the vendored
 *   registry compositions — see compositions/REGISTRY-NOTICE.md.)
 *
 * Consumed by services/studioFontsService.ts for the embedded Studio editor's
 * font picker. RE-SYNC THIS FILE when bumping @hyperframes/* — diff against
 * packages/core/src/fonts/systemFontLocator.ts at the new tag.
 *
 * Local changes vs upstream: this header + a guarded candidate access in
 * `pickBestCandidate` (repo lint forbids non-null assertions).
 */
...
function pickBestCandidate(candidates: FontCandidate[]): LocatedFont | null {
  ...
  // Local change vs upstream: guarded access instead of a non-null assertion
  // (repo lint forbids `!`); the empty-array early return is subsumed.
