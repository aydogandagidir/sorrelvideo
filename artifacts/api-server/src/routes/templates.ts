 ...
 * from the vendored registry manifest (plus the hand-authored showcase templates)
 * so the thumbnails route only ever serves a known template's poster (an
 * allow-list — never an arbitrary `:slug` off disk).
 */
const THUMBNAIL_SLUGS = new Set([
  ...REGISTRY_TEMPLATES.map((t) => t.slug),
  // Hand-authored (M-features Track B/D/F+) — PNGs generated by generate-thumbnails.mjs.
  "product-3d",
  "lottie-reveal",
  "video-spotlight",
  "talking-host",
...
const PREVIEW_FALLBACKS: Record<string, string> = {
  ...
  // talking-host's gallery preview runs its built-in DEMO narration (~9s) when
  // host.payload is unsubstituted; the duration matches that demo (and
  // STUDIO_FALLBACKS) so data-duration never leaks a raw placeholder.
  duration: "9",
};
